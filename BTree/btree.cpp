...
int randomInsAndDel(){
	...
	cout<< "#####randomInsAndDel success !######\n" ;
	return 0;
}


int sequenceInsAndDel(){
	Btree<int> btree;
	for (int i = 0; i < 150000; ++i){
		bool rt  = btree.insert(i);
		if (!rt){
			cout << "insert "<< i << "ERR\n";
			return 0;
		}
	...
	for (int i = 0; i < 149999; ++i){
		bool rt = btree.del(i);  
		if (!rt){
			cout<<"del ERR can't delete "<< i << "  "<< rt <<"  \n";
			return 0;
		};
					
	}
	cout<< "#####sequenceInsAndDel success !######\n" ;
